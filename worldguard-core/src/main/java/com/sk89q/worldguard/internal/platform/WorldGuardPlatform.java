...
public interface WorldGuardPlatform {
    ...
    /**
     * Optional command for a region-info menu button. Placeholders {world}, {region_name}, {player} are replaced by the platform.
     * If null, core uses the default (e.g. /rg addowner -w "world" regionId ).
     *
     * @param type       which button (ADD_OWNER, ADD_MEMBER, REMOVE_OWNER, REMOVE_MEMBER)
     * @param world      world name
     * @param regionId   region id
     * @param playerName name of the player viewing the menu (may be null)
     * @return command string for SUGGEST_COMMAND, or null to use default
     */
    @Nullable
    default String getRegionInfoCommand(RegionInfoCommandType type, String world, String regionId, @Nullable String playerName) {
        return null;
    }

    /**
     * Whether the region-info button command should be run as the player (true) or suggested into chat (false).
     * Only used when a custom command is set for that type.
     */
    default boolean isRegionInfoCommandRun(RegionInfoCommandType type) {
        return false;
    }
}
