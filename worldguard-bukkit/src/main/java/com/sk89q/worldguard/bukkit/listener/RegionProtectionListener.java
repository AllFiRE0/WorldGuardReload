...
public class RegionProtectionListener extends AbstractListener {
    ...
    private static final String DENY_SOUND_KEY = "worldguard.region.lastDenySound";
    ...
    private void tellErrorMessage(DelegateEvent event, Cause cause, Location location, String what) {
        ...
        if (rootCause instanceof Player) {
            ...
            playDenySoundIfAllowed(player, location, query, localPlayer, now);
        }
    }

    private void playDenySoundIfAllowed(Player player, Location location, RegionQuery query, LocalPlayer localPlayer, long now) {
        BukkitConfigurationManager cfg = getConfig();
        if (!cfg.isDenySoundEnabled()) {
            return;
        }
        Boolean denySoundFlag = query.queryValue(BukkitAdapter.adapt(location), localPlayer, Flags.DENY_SOUND);
        if (denySoundFlag == null || !denySoundFlag) {
            return;
        }
        if (!player.hasPermission("worldguard.sound")) {
            return;
        }
        long cooldownMs = cfg.getDenySoundCooldownTicks() * 50L;
        Long lastSound = WGMetadata.getIfPresent(player, DENY_SOUND_KEY, Long.class);
        if (lastSound != null && now - lastSound < cooldownMs) {
            return;
        }
        WGMetadata.put(player, DENY_SOUND_KEY, now);
        SoundCategory category;
        try {
            category = SoundCategory.valueOf(cfg.getDenySoundSource().toUpperCase());
        } catch (IllegalArgumentException e) {
            category = SoundCategory.MASTER;
        }
        player.playSound(location, cfg.getDenySoundName(), category, cfg.getDenySoundVolume(), cfg.getDenySoundPitch());
    }
    ...
    static void formatAndSendDenyMessage(String what, LocalPlayer localPlayer, String message) {
        ...
        localPlayer.sendProtectionMessage(message.replace("%what%", what));
    }
