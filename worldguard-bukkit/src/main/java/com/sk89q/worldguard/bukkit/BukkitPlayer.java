...
public class BukkitPlayer extends com.sk89q.worldedit.bukkit.BukkitPlayer implements LocalPlayer {
    ...
    @Override
    public void sendProtectionMessage(String message) {
        if (silenced || message == null || message.isEmpty()) return;
        if (!(WorldGuard.getInstance().getPlatform().getGlobalStateManager() instanceof BukkitConfigurationManager)) {
            printRaw(message);
            return;
        }
        ProtectionMessageDisplay display = ((BukkitConfigurationManager) WorldGuard.getInstance().getPlatform().getGlobalStateManager()).getProtectionMessageDisplay();
        String translated = ChatColor.translateAlternateColorCodes('&', message);
        Player p = getPlayer();
        switch (display) {
            case ACTION_BAR:
                sendActionBar(p, translated);
                break;
            case TITLE:
                if (WorldGuard.getInstance().getPlatform().getGlobalStateManager().get(getWorld()).forceDefaultTitleTimes) {
                    p.sendTitle(translated, "", 10, 70, 20);
                } else {
                    p.sendTitle(translated, "", -1, -1, -1);
                }
                break;
            case CHAT:
            default:
